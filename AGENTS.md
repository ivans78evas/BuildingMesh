...
## 5. REACTIVE AGENT PROTOCOL (ReAct)
Агент Jules ОБЯЗАН работать в режиме ReAct (Reason + Act). Каждая сложная задача разбивается на итерации:
- **Thought (Рассуждение):** Анализ текущей ситуации, архитектурных ограничений и недостающих данных.
- **Action (Действие):** Вызов конкретного инструмента (компиляция, поиск, запись файла).
- **Observation (Наблюдение):** Анализ результата действия (логи ошибок, вывод тестов).
- **Loop:** Возврат к анализу до достижения финального состояния. Запрещено выдавать невалидный или некомпилируемый код.
